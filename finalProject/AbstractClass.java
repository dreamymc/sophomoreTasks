import java.util.List;

abstract class AbstractClass {
    public abstract void addPatient();

    public abstract void displayQueue();

    public abstract void displayAllPatients();

    public abstract void searchPatient();

    public abstract String medCondition(int x);

    public abstract String severity(int y);

    public abstract void queueMapSorting();

    public abstract List<Integer> exchangeSort(List<Integer> arr);

}
